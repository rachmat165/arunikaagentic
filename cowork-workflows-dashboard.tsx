...
import { COWORK_INSTANCES } from '@/services/cowork-api';
...
export function CoworkWorkflowsDashboard() {
  ...
  const executeDailyWorkflows = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        alert(
          `Daily workflows executed: ${data.summary.successful} successful, ${data.summary.failed} failed`
        );
        setTimeout(fetchStats, 2000);
      }
    } catch (error) {
  ...
  const displayWorkflows: WorkflowData[] =
    workflows.length > 0
      ? workflows
      : [
          {
          ...
          {
            id: 'daily-briefing',
            name: 'Daily Briefing & Intelligence Report',
            description: 'Generate executive briefing dengan key metrics & insights',
            instance: 'ceo-dashboard',
            frequency: 'Daily at 08:30 WIB',
            status: 'active',
            stats: {
              totalExecutions: 0,
              successCount: 0,
          ...
          },
          {
            ...
            description: 'Compile weekly metrics & performance analysis',
            instance: 'ceo-dashboard',
            frequency: 'Every Monday at 17:00 WIB',
            status: 'active',
            stats: {
              totalExecutions: 0,
              successCount: 0,
          ...
          },
          {
            ...
            name: 'Monthly Financial Summary Report',
            description: 'Compile monthly P&L, cash flow & balance sheet',
            instance: 'ceo-dashboard',
            frequency: 'Last day of month at 17:00 WIB',
            status: 'active',
            stats: {
              totalExecutions: 0,
              successCount: 0,
  ...
  const totalSuccess = Object.values(stats).reduce((sum, s) => sum + s.successCount, 0);
  const totalFailure = Object.values(stats).reduce((sum, s) => sum + s.failureCount, 0);
  const totalExecutions = Object.values(stats).reduce((sum, s) => sum + s.totalExecutions, 0);
  const successRate = ((totalSuccess / (totalExecutions || 1)) * 100).toFixed(1);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white">
            🤖 Cowork Arunika Agentic AI Workflows
          </h2>
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            Monitor & execute {workflows.length > 0 ? workflows.length : '12'} workflows across 4 Cowork
      ...
      {mainTab === 'approval-system' && <ApprovalWorkflowDashboard />}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {filteredWorkflows.length > 0 ? (
              filteredWorkflows.map((workflow) => {
                ...
                const currentSuccessRate = workflowStats?.successRate || 0;
                          ...
                          <div className="flex justify-between items-center text-sm">
                            ...
                            <span
                              className={`font-semibold ${
                                currentSuccessRate >= 90 ? 'text-green-600' : 'text-orange-600'
                              }`}
                            >
